...
"""
One QUIC stream, as an `IO`. Bytes arrive from msquic as chunks on `inbox`; `read` drains
...
mutable struct Stream <: IO
    ...
    unidirectional::Bool
    inbox::Channel{Any}                  # (:data, bytes, fin) | (:reset, code)
    started::Channel{Int}
    rbuf::Vector{UInt8}
    rpos::Int
...
Stream(handle, unidirectional) = Stream(handle, -1, unidirectional, Channel{Any}(Inf), Channel{Int}(1), UInt8[], 1, false, true)

mutable struct Connection
    handle::HQUIC
    config::HQUIC
    events::Channel{Any}                 # (:connected,) | (:shutdown, why)
...
Connection(config) = Connection(C_NULL, config, Channel{Any}(Inf), Channel{Stream}(Inf), Channel{Vector{UInt8}}(Inf), Channel{Nothing}(1), true, 0)
...
function on_connection(::HQUIC, ctx::Ptr{Cvoid}, ev::Ptr{Cvoid})::Status
    ...
    try
        ...
        elseif t == MsQuic.SHUTDOWN_INITIATED_BY_PEER
            put!(conn.events, (:shutdown, (:peer, payload(MsQuic.EvShutdownByPeer, ev).ErrorCode)))
...
function on_stream(::HQUIC, ctx::Ptr{Cvoid}, ev::Ptr{Cvoid})::Status
    ...
    try
        ...
        elseif t == MsQuic.RECEIVE
            ...
            put!(s.inbox, (:data, MsQuic.copy_buffers(e.Buffers, e.BufferCount), e.Flags & MsQuic.RECEIVE_FLAG_FIN != 0))
...
function connect(host::AbstractString, port::Integer; alpn = "h3", verify = true, timeout = 5.0, kw...)
    ...
    if ev != (:connected,)
        close(conn)
        throw(ConnectError(ev === nothing ? "timeout after $(timeout)s" : string(ev[2])))
    end
    conn
end
...
function fill!(s::Stream)
    ...
    ev = try take!(s.inbox) catch; (:data, UInt8[], true) end
    if ev[1] == :reset
        s.fin = true
        throw(StreamReset(s.id, ev[2]))
    end
    bytes, fin = ev[2], ev[3]
    if s.rpos > length(s.rbuf)
        s.rbuf = bytes; s.rpos = 1
    else
        append!(s.rbuf, bytes)
    end
    fin && (s.fin = true)
...
function Base.readavailable(s::Stream)
    bytesavailable(s) == 0 && fill!(s)
    out = s.rbuf[s.rpos:end]
    s.rpos = length(s.rbuf) + 1
    out
end

Base.unsafe_write(s::Stream, p::Ptr{UInt8}, n::UInt) = (MsQuic.stream_send(s.handle, unsafe_wrap(Vector{UInt8}, p, n) |> copy); Int(n))
